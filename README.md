...
Для обеспечения наибольшей безопасности предполагается использовать следующие инструменты:

- **SSH-ключи** для более безопасного подключения к серверу
- **fail2ban** для блокировки хостов, которые делают множественные попытки входа на ваш сервер.
- **ufw** в качестве инструмента для работы с iptables/nftables (фаервол). 

Если данные инструменты у вас уже настроены - переходите к следующему разделу.
...
### SSH-ключи (сервер)
Скрипт содержит базовую настройку ssh, fail2ban и ufw, для этого необходимо перед запуском скрипта (4ый в шаге 1 и шаге 2) выполнить следующую инструкцию
```bash
sudo ./scripts/setup.sh security
```
### Домен (опционально)
...
4. ```bash <a name="test"></a>
   ./scripts/setup.sh exit
...
- Добавит порты в ufw (при наличии)
