...
**Что такое Reality?**
XTLS-Reality маскирует VPN-протокол под обычное TLS-соединение к легитимному сайту (например, microsoft.com). DPI не может отличить такой трафик от обычного HTTPS.

**Зачем тогда два сервера?**
Reality прячет протокол, но IP-адрес назначения не шифруется никогда — это базовое свойство сети. Без ретранслятора DPI видит, что вы держите постоянное TLS-соединение с датацентром в Нидерландах, да ещё и с чужим SNI — а это повод для анализа и блокировки по IP.

С ретранслятором картина другая: клиент подключается к российскому IP, SNI можно подобрать от российского же сайта — для наблюдателя это выглядит как обычный внутренний HTTPS-трафик.

![Сравнение: с ретранслятором и без](./docs/vpn_relay_comparison.svg)

Если коротко: **Reality прячет «что» (протокол), ретранслятор прячет «куда» (зарубежный IP).**

Практический бонус: если зарубежный IP заблокируют, без ретранслятора придётся перенастраивать всех клиентов. А с ним — просто меняешь exit-сервер в одном месте, клиенты ничего не заметят.

### SelfSteal SNI (опционально)

Стандартный Reality маскирует трафик под обращение к чужому сайту — например, microsoft.com. Но IP-адрес вашего сервера не принадлежит Microsoft, и продвинутая DPI может это заметить.

SelfSteal решает эту проблему: вы указываете свой домен, и сервер хостит под ним настоящий сайт через Caddy. DNS-запись домена указывает на IP сервера — для наблюдателя всё сходится: SNI, IP, сертификат, сайт.

![SelfSteal: сравнение](./docs/selfsteal-comparison.svg)

SelfSteal полностью опциональный — если вы не хотите настраивать домен, просто нажмите Enter при установке и всё будет работать как раньше (с маскировкой под microsoft.com или подобный сайт).

Если SelfSteal включён, Caddy также проксирует панель управления и подписки — доступ к ним идёт через домен без указания порта.

![Архитектура с SelfSteal](./docs/selfsteal-architecture.svg)

### DNS-фильтрация рекламы

Exit-сервер использует AdGuard DNS для фильтрации рекламы и трекеров на уровне DNS. Рекламные домены (трекеры Meta, Google Ads и т.д.) блокируются автоматически — клиентам ничего настраивать не нужно.
...
- **Домен для SelfSteal** (опционально) — для маскировки трафика под собственный сайт. Нужны A-записи для 2-4 поддоменов, указывающие на ваши серверы. VPN работает и без SelfSteal, но с ним маскировка надёжнее
...
> **Повторный запуск:** Если exit-сервер уже настроен, скрипт предложит использовать `update-exit` вместо повторной установки. Для принудительной переустановки: `sudo ./scripts/setup.sh exit --force`
>
> **Свой SSH:** Если вы уже настроили SSH под себя (порт, 2FA, и т.д.) и не хотите, чтобы скрипт менял конфигурацию — добавьте `--skip-ssh`
...
Если указать домен (например, `exit.example.com`), скрипт дополнительно:
- Установит Caddy и настроит его на Unix-сокет
- Выпустит SSL-сертификат автоматически
- Спросит откуда взять статический сайт (Git-репо / локальный путь / сгенерировать заглушку)

Если нажать Enter — будет использоваться стандартная маскировка под популярный сайт.

> **Важно:** Если используете SelfSteal, DNS A-запись для домена должна указывать на IP сервера **до** запуска скрипта.

Затем скрипт автоматически:
- Обновит систему и установит зависимости
- Установит XRAY-core
- Выберет лучший Reality-сайт для маскировки (или настроит SelfSteal)
...
> **Повторный запуск:** Если relay-сервер уже настроен, скрипт предложит использовать `update-relay`. Для принудительной переустановки: `sudo ./scripts/setup.sh relay --force` (все ключи будут пересозданы, клиенты потеряют доступ).
>
> **Свой SSH:** Добавьте `--skip-ssh`, чтобы пропустить SSH hardening (флаги можно комбинировать: `--force --skip-ssh`)
...
Затем запросит настройки панели и (опционально) SelfSteal:

```
=== Relay Configuration ===
3X-UI panel port [41532]:                                  ← Enter
...
**Если указан домен для SelfSteal**, скрипт дополнительно спросит:
...
Static site for SelfSteal:
  1) Git repo URL
  2) Local path on server
  3) Generate placeholder page
Choose [1/2/3] (default: 3):                               ← выбор контента
```

**Если нажали Enter** — скрипт спросит только домен для подписок (как раньше):
```
Domain for subscriptions, optional, Enter to skip:         ← домен или Enter
```
...
- Если SelfSteal: установит Caddy, настроит проксирование панели и подписок
...
**Если используете SelfSteal**, настройте A-записи **до запуска** скриптов:

```
exit.example.com   → 185.x.x.x  (IP exit-сервера)
example.com        → 91.x.x.x   (IP relay-сервера, основной домен)
panel.example.com  → 91.x.x.x   (IP relay-сервера, панель)
sub.example.com    → 91.x.x.x   (IP relay-сервера, подписки)
```

**Если SelfSteal не используете**, но указали домен для подписок:

```
vpn.example.com → 91.x.x.x  (IP вашего relay-сервера)
```

Если домен не указывали — этот шаг не нужен.
...
Перед обновлением автоматически создаётся резервная копия конфигурации. Если XRAY или 3X-UI не запустится после обновления — предыдущий конфиг восстанавливается автоматически.
...
При SelfSteal-режиме `--upgrade` также обновляет Caddy.

Все команды принимают `--skip-ssh` для пропуска SSH hardening (полезно, если SSH уже настроен вручную).

### Удаление

Для полной очистки сервера (удаляет 3X-UI, XRAY, Caddy, fail2ban, UFW):
...
| DPI bypass | Фрагментация TLS ClientHello затрудняет анализ трафика системами DPI |
| Routing | Блокировка доступа к внутренним сетям (127.0.0.1, 10.x, 192.168.x) через VPN |
| Caddy (SelfSteal) | Автоматические сертификаты, проксирование панели, сайт-заглушка для DPI |
| SSL | Автоматический сертификат Let's Encrypt для домена подписок |

## Устранение неполадок

**Логи установки и обновления:**
```bash
# Все setup/update команды сохраняют вывод в лог-файл
