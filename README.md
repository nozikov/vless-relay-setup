...
#### WireGuard-бэкхол к внешнему пиру (опционально)

Вместо exit-VPS, настроенного этим проектом, relay может ходить в интернет через **WireGuard-туннель к внешнему, самостоятельно управляемому пиру** — например, домашнему роутеру (UniFi Express и т.п.), который уже умеет терминировать WireGuard. Egress-IP при этом становится резидентным, а не датацентровым. Режимы взаимоисключающие: relay работает либо через VLESS-бэкхол к exit-VPS (по умолчанию), либо через WireGuard-пир.

Ключевые отличия от VPS-режима:

- **Ничего не ставится на стороне роутера** — пир создаётся вручную в его собственном UI, скрипты этого репозитория его не трогают. Никаких новых пакетов и сервисов не появляется и на relay: используется userspace `wireguard`-outbound самого XRAY (без ядерного модуля, `wg-quick` и нового интерфейса).
- **Direct Exit / CDN Fallback / Hysteria 2 недоступны** — эти каналы предполагают VPS, доступный извне на 443. В подписке остаётся только основной relay-канал.
- **Нет AI-роутинга и exit-side инспекции** — на том конце нет XRAY-процесса вообще, только NAT-шлюз.

Выбор режима — первый вопрос `setup-relay.sh`. Параметры пира можно задать двумя способами:

1. **Импорт `.conf`** (предпочтительно): UniFi отдаёт стандартный `wg-quick`-конфиг — передайте путь в промпте или флагом `--wg-conf <path>`. Строка `DNS =` игнорируется (у XRAY-outbound нет in-tunnel DNS), отсутствующие поля получают дефолты (keepalive 25, MTU 1380).
2. **Ручной ввод**: приватный ключ relay, туннельный адрес (например `192.168.2.4/32` — это подсеть туннеля, не LAN роутера), публичный ключ роутера, endpoint `host:port` (обычно DDNS-имя из-за динамического резидентного IP).

Перед записью конфигурации скрипт делает **pre-flight**: поднимает одноразовый userspace-туннель (`xray run` с scratch socks-инбаундом) и проверяет реальный handshake + egress через `curl`. Перепутанный ключ или неверный endpoint падает сразу, до установки 3X-UI и записи шаблона — «слепая» UDP-проверка тут бесполезна, WireGuard игнорирует не-handshake пакеты. Тот же dry-run гоняется в `selfcheck` (зелёная галочка подтверждает egress, но **не** LAN-scoping роутера и не живое состояние работающего outbound — интерфейса `wg0` и `wg show` на relay нет).

**Безопасность (важнее, чем в VPS-случае):** «exit» здесь — домашний роутер с живой LAN (принтеры, NAS, IoT, админка). Защита двухслойная, и оба слоя должны совпадать:

1. **Relay-side блок `geoip:private`** — ставится автоматически: приватные адреса через туннель недоступны, кроме явно разрешённых.
2. **Scoping пира на роутере** — целиком ваша ответственность: ограничьте peer в UI роутера WAN-egress'ом плюс ровно теми LAN-хостами, которые нужны удалённо.

Для точечного удалённого доступа к устройствам за роутером (например, админка UniFi на `192.168.2.1`) есть **LAN-exception список**: промпт при установке или `--wg-lan-allow 192.168.2.1` в `update-relay`. Только узкие исключения (`/32`, максимум `/24`) — целые приватные диапазоны (`/16`, `/8`) отклоняются. По умолчанию список пуст. Разрешённый на relay хост реально доступен только если роутер-side scoping его тоже пропускает.

Управление через `update-relay`:

```bash
# Обычный ре-ран: режим и параметры (включая LAN-exceptions) читаются из живого шаблона
sudo ./scripts/setup.sh update-relay

# Обновить параметры пира / перейти с VPS на WireGuard
sudo ./scripts/setup.sh update-relay --wg-conf /root/wg-peer.conf
sudo ./scripts/setup.sh update-relay --wg-backhaul <priv_key> <address> <peer_pubkey> <endpoint>

# Настроить LAN-exceptions отдельно от ключей
sudo ./scripts/setup.sh update-relay --wg-lan-allow 192.168.2.1
sudo ./scripts/setup.sh update-relay --wg-lan-allow none   # заблокировать всё приватное

# Вернуться на VPS-exit (параметры прежнего режима не сохраняются — нужен полный набор)
sudo ./scripts/setup.sh update-relay --vless-backhaul <exit_ip> <exit_uuid> <exit_pubkey> <exit_short_id> <exit_sni>
```

Переключение режима требует полного набора параметров целевого режима: после перезаписи шаблона параметры прежнего режима нигде не хранятся. Pre-flight гоняется и в `update-relay` — при неудаче обновление отменяется, текущий шаблон не трогается.
...
| `--wg-conf` | setup-relay, update-relay | Импорт WireGuard `.conf` (wg-quick/UniFi) для WG-бэкхола |
| `--wg-backhaul` | update-relay | Включить/обновить WG-бэкхол: `<priv_key> <address> <peer_pubkey> <endpoint> [allowed_ips] [keepalive] [mtu]` |
| `--vless-backhaul` | update-relay | Вернуться на VPS-exit: `<exit_ip> <exit_uuid> <exit_pubkey> <exit_short_id> <exit_sni>` |
| `--wg-lan-allow` | update-relay | LAN-exception список для WG-бэкхола (`none` — очистить) |
